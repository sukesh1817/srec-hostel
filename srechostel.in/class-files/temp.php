...
class session
{
    ...
    public function __construct($id = "none", $whoIs = "none")
    {
    ...
    public function updateSession($id, $staffPass)
    {
        $conn = new Connection();
        $sqlConn = $conn->returnConn();
        ...
        $sessionId = md5($id . $staffPass . $ip . $currentTime);
        if ($this->whoIs == "Staff") {
    ...
    public function createSession($id, $staffPass)
    {
        $conn = new Connection();
        $sqlConn = $conn->returnConn();
        $ip = $_SERVER["REMOTE_ADDR"];
