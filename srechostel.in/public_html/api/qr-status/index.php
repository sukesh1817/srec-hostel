...
echo $htmlContent;
                ...
                $entry = $pass->entryThePass($rollNo, "working_days_pass");
                if ($entry) {
                    header("Content-Type:application/json");
                    echo $entry;
                } else {
                ...
                $entry = $pass->entryThePass($rollNo, "general_home_pass");
                if ($entry) {
                    header("Content-Type:application/json");
                    echo $entry;
                } else {
            ...
            if ($passBooked[0]) {
                header("Content-Type:application/json");
                echo '{"Message":"admin not accepted out pass","action":"failure"}';
            } else if ($passBooked[1]) {
                header("Content-Type:application/json");
                echo '{"Message":"admin not accepted working day pass","action":"failure"}';
            } else if ($passBooked[2]) {
                header("Content-Type:application/json");
                echo '{"Message":"admin not accepted general home pass","action":"failure"}';
            } else {
                header("Content-Type:application/json");
                echo '{"Message":"Something went wrong","action":"failure"}';
            }
        ...
        header("Content-Type:application/json");
        echo '{"Message":"none of the pass is booked","action":"failure"}';
    }
} else {
    header("Content-Type:application/json");
